    ...
    <div className="h-full bg-[#FEFFF9] flex flex-col overflow-hidden animate-fade-in relative">
      ...
      {successModal.show && (
        <div 
          ...
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh' }}
        >
          <div className="bg-white rounded-[44px] w-full max-w-[340px] p-10 shadow-[0_25px_70px_rgba(0,0,0,0.5)] border border-white/20 flex flex-col items-center text-center animate-modal-pop" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="w-full space-y-4">
              {successModal.type === 'sync' ? (
                <button onClick={() => navigate('/shopping-list')} className="w-full py-5 bg-[#FF5C00] text-white rounded-[28px] font-black text-[17px] shadow-xl shadow-orange-500/20 active:scale-95 transition-all flex items-center justify-center gap-3 border-b-6 border-[#E65100]">
                  前往购物车清单 <ArrowRight size={20} />
                </button>
              ) : (
                <button onClick={() => navigate('/calendar')} className="w-full py-5 bg-[#FF5C00] text-white rounded-[28px] font-black text-[17px] shadow-xl shadow-orange-500/20 active:scale-95 transition-all flex items-center justify-center gap-3 border-b-6 border-[#E65100]">
                  前往日历查阅 <ArrowRight size={20} />
                </button>
              )}
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar smooth-scroll px-6 py-6 pb-48 space-y-12 relative z-10">
          ...
          <div className={`grid gap-4 ${queuedRecipes.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
            {queuedRecipes.map((recipe) => (
              <div key={recipe.id} className="relative group animate-fade-in">
                <div 
                  className="w-full aspect-[3/4] bg-white border-2 border-[#F0E6D2] rounded-[36px] overflow-hidden shadow-xl shadow-[#B45309]/5 relative cursor-zoom-in active:scale-[0.98] transition-all duration-300"
        ...
        <section className="space-y-5">
          ...
          <div className="bg-white border-2 border-[#F0E6D2] rounded-[40px] p-7 shadow-sm shadow-[#B45309]/5">
            <div className="flex flex-wrap gap-3">
              {aggregatedIngredients.map((ing, idx) => (
                <div 
                  key={idx} 
                  className="bg-[#FFF9E8]/80 hover:bg-[#FFF9E8] border border-[#F0E6D2]/60 rounded-full pl-5 pr-3 py-3 flex items-center gap-3 transition-all active:scale-95 shadow-sm"
                >
              ...
              {aggregatedIngredients.length === 0 && (
                <div className="w-full py-8 text-center">
                  <p className="text-[13px] font-bold text-[#B45309]/30">暂无食材统计萝...</p>
                </div>
              )}
            </div>
          </div>
            ...
            {queuedRecipes.map((recipe, rIdx) => (
              <div key={recipe.id} className="space-y-8 relative">
                ...
                <div className="flex items-center gap-5 px-1 relative z-10">
                   <div className="w-20 h-20 rounded-[28px] overflow-hidden shadow-xl shadow-[#B45309]/10 border-2 border-white shrink-0">
                ...
                <div className="space-y-6 relative z-10">
                  {recipe.steps.map((step, sIdx) => (
                    <div key={sIdx} className="bg-white p-7 rounded-[36px] border-2 border-[#F0E6D2] shadow-sm flex flex-col gap-5 active:scale-[0.99] transition-transform">
                       ...
                       {step.image && (
                         <div className="rounded-[28px] overflow-hidden border border-[#F0E6D2] aspect-[16/9] cursor-zoom-in" onClick={() => setZoomedImage(step.image!)}>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-8 pb-[calc(env(safe-area-inset-bottom)+24px)] bg-gradient-to-t from-[#FEFFF9] via-[#FEFFF9]/90 to-transparent shrink-0 z-[90] pointer-events-none">
        <button 
          ...
          className="w-full bg-[#FF5C00] text-white py-6 rounded-[32px] font-black text-xl shadow-[0_20px_50px_rgba(255,92,0,0.4)] active:scale-[0.98] transition-all border-b-6 border-[#E65100] flex flex-col items-center justify-center gap-1 pointer-events-auto"
        >
          {isFinishing ? (
            <div className="flex items-center gap-4">
