...
import { 
  ...
  Utensils, Coffee, Pizza, Soup, BookOpen, Plus, Search
} from 'lucide-react';
...
const Calendar: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-[#FEFFF9] relative overflow-hidden">
      <header className="px-6 pt-12 pb-4 shrink-0 animate-fade-in bg-white/50 backdrop-blur-md border-b border-[#F0E6D2]/20">
        ...
        <div className="flex justify-between items-center bg-white p-1.5 rounded-[24px] border border-[#F0E6D2] shadow-sm">
           {weekDays.map(date => {
      ...
      <div className="flex-1 overflow-y-auto smooth-scroll no-scrollbar px-6 pt-4 pb-40 space-y-3">
        {Object.entries(MEAL_TYPES).map(([type, meta]) => {
          ...
          return (
            <div key={type} className="animate-fade-in">
              {recipe ? (
                <div className="bg-white p-3 rounded-[24px] border border-[#F0E6D2] shadow-sm flex items-center gap-3 relative overflow-hidden group">
                  <div className={`absolute left-0 top-0 bottom-0 w-1 bg-${meta.color.split('-')[1]}-400`}></div>
                  ...
                  <div className="flex-1 min-w-0" onClick={() => navigate(`/recipe/${recipe.id}`)}>
                    <p className="text-[10px] font-bold text-[#B45309]/30 uppercase mb-0.5">{meta.label}</p>
                    <p className="font-black text-[#5D3A2F] text-[15px] truncate">{recipe.name}</p>
                  </div>
                  <button onClick={() => removeMealPlan(plan.id)} className="p-2 text-[#B45309]/10 hover:text-red-500 transition-colors">
                    <Trash2 size={16} />
                  </button>
                </div>
              ) : (
                <button 
                  ...
                  className="w-full bg-[#FEFFF9] border border-dashed border-[#F0E6D2] rounded-[24px] p-3 flex items-center gap-3 active:bg-[#FFF9E8] transition-all"
                >
                  <div className={`w-10 h-10 bg-white border border-[#F0E6D2] rounded-xl flex items-center justify-center shrink-0`}>
                    <Plus size={18} className="text-[#FF5C00]" />
                  </div>
                  <p className="text-[12px] font-black text-[#B45309]/30">点击加餐 {meta.label} 萝</p>
                </button>
              )}
            </div>
          );
      ...
      {isPickingRecipe && (
        <div className="fixed inset-0 z-[1000] bg-black/60 backdrop-blur-xl flex items-end justify-center animate-fade-in">
          <div className="bg-[#FEFFF9] w-full max-w-[430px] h-[75%] rounded-t-[50px] p-8 animate-slide-up flex flex-col shadow-2xl border-t border-white/40">
              ...
              {recipes.length > 0 ? (
                recipes.map(r => (
                  <button 
                    ...
                    className="w-full bg-white border border-[#F0E6D2] p-3 rounded-[24px] flex items-center gap-4 active:scale-[0.98] transition-all shadow-sm"
                  >
